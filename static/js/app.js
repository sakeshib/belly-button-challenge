...
function buildMetadata(sample) {
  d3.json("https://static.bc-edx.com/data/dl-1-2/m14/lms/starter/samples.json").then((data) => {
    ...
    const result = metadata.filter(object => object.id == sample)[0];
...
function buildCharts(sample) {
  d3.json("https://static.bc-edx.com/data/dl-1-2/m14/lms/starter/samples.json").then((data) => {
    ...
    const result = samples.filter(object => object.id === sample)[0];
    ...
    const sortedData = sample_values.map((value, index) => ({
      value: value,
      id: otu_ids[index],
    ...
    // Top 10 values and reverse for horizontal bar chart
